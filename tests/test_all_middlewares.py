...
class TestABTesting:
    ...
    def test_ab_test_sticky_variant(self):
        ...
        app.add_middleware(
            ABTestMiddleware,
            experiments=[Experiment(name="exp", variants=["x", "y"])]
        )
        client = TestClient(app)
...
class TestAPIVersionHeader:
    def test_api_version_header(self):
        ...
        response = client.get("/")
        assert response.status_code == 200
        assert "X-API-Version" in response.headers or response.headers.get("x-api-version") == "1.0.0"
...
class TestContentNegotiation:
    def test_content_negotiation(self):
        ...
        async def homepage(request):
            return JSONResponse({"type": getattr(request.state, "content_type", "application/json")})
...
class TestDeprecation:
    def test_deprecation_warning(self):
        ...
        info = DeprecationInfo(message="This endpoint is deprecated", sunset_date="2025-12-31", replacement="/new")
        app.add_middleware(
            DeprecationMiddleware,
            deprecated_paths={"/old": info}
        )
        client = TestClient(app)
...
class TestFeatureFlag:
    def test_feature_flag(self):
        ...
        app.add_middleware(
            ...
            flags={"new_feature": True, "old_feature": False}
        )
        client = TestClient(app)
...
class TestHeaderTransform:
    def test_header_transform(self):
        ...
        app.add_middleware(
            ...
            add_response_headers={"X-Custom": "value"}
        )
        client = TestClient(app)
...
class TestUserAgent:
    def test_user_agent(self):
        ...
        response = client.get("/", headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"})
        assert response.status_code == 200
